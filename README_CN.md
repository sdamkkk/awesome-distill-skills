...
- ✅ **[leijun-distill-skill](examples/leijun/)** - 雷军深度认知蒸馏。多源语料收集 → 分批提炼 → 结构化知识卡 → Skill 生成 → 质量验证。`认知框架` `中文`
  - 蒸馏对象：雷军
  - 亮点：结合 nuwa-skill 方法论 + 透明调研过程 + 证据分级。v1.1 System Prompt（458 行，30KB）+ 57 条知识卡 + 验证报告（8.4/10）。**[完整复盘 →](case-studies/leijun-distillation.md)**
...
- **[雷军（Lei Jun）— 完整蒸馏成果](examples/leijun/)** — 小米创始人雷军认知操作系统的四步完整蒸馏。包含 System Prompt v1.1（458 行，30KB）、57 条结构化知识卡、验证报告（评分 8.4/10）。将 `leijun-skill.md` 复制到任何 LLM 的 System Prompt 即可使用。`认知框架` `中文` `v1.1`

> 📦 想贡献一个示例？请包含最终 Skill 文件 + 知识卡 + 验证报告。[提交 PR](CONTRIBUTING.md)！
